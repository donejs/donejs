...
exports.add = function(root, name, params) {
  var moduleName = 'donejs-' + name;

  return Q.ninvoke(npm, 'load', { loaded: false})
    .then(installIfMissing(root, moduleName))
    .then(function() {
      var plugin = require(path.join(root, moduleName));
      var exportName = params && params[0] || 'default';
      return plugin && plugin[exportName]();
    });
};
